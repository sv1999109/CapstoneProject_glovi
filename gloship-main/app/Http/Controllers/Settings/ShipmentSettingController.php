...
class ShipmentSettingController extends Controller
{
    ...
    {
        $data['page_title'] = trans_choice('messages.Order', 2);
        return view(get_theme_dir('shipment.order.list', 'dashboard'), $data);
    }
    ...
    public function invoice_view($id)
    {
        $model = Orders::where('invoice_id', $id)->first();
        if ($model) {
            $data['page_title'] = __('messages.Invoice') . ': ' . $model->invoice_id;
    ...
    public function invoice_datatable($type = null, $userid, Request $request)
    {
        if ($request->ajax()) {
            ...
            //moderators/admins
            if ($role >= '4') {
                $shipments = Orders::orderByDesc('created_at');

                //filter by user
                if ($type == 'user') {
                    $shipments = Orders::where('owner_id', $userid)->orderByDesc('created_at');
                }
            }
            //staffs
            if ($role == '3') {
                $shipments = Orders::whereRaw("from_branch = '$branch' OR to_branch = '$branch'")->orderByDesc('created_at');
            ...
            //Delivery Agents
            if ($role == '2') {
                $shipments = Orders::whereRaw("delivery_agent = '$client'")->orderByDesc('created_at');
            }
            //customers
            if ($role == '1') {
                $shipments = Orders::whereRaw("owner_id = '$client'")->orderByDesc('created_at');
            }

            return DataTables::of($shipments)
                ->addIndexColumn()
                ->addColumn('action', function (Orders $shipment) {
                    $actionBtn = '-';
                    //user role settings
                    $role = Auth()->user()->role;
                    $client = Auth()->user()->id;
                    ...
                    $payment_url = "<a target='_blank'  class='btn btn-sm btn-outline-primary m-1' href='{$url}'><i class='fa fa-eye'></i></a>";
                    return $payment_url;
                })
                ->editColumn('invoice_id', function (Orders $shipment) {
                    $url = route('dashboard.shipments.invoice', ['id' => $shipment->invoice_id]);
                    $actionurl = "<a target='_blank' href='{$url}'>{$shipment->invoice_id}</a>";
                    return $actionurl;
                })
                ->editColumn('code', function (Orders $shipment) {
                    $url = route('dashboard.shipments.view', ['id' => $shipment->code]);
                    $actionurl = "<a href='{$url}'>{$shipment->code}</a>";
                    return $actionurl;
                })

                ->editColumn('payment_status', function (Orders $shipment) {
                ...
                ->editColumn('owner_id', function (Orders $shipment) {
                ...
                ->editColumn('shipping_cost', function (Orders $shipment) {
                    $shipping_cost = get_money($shipment->shipping_cost, $shipment->currency, 'symbol', 'localize');
                    return $shipping_cost;
                })

                ->editColumn('created_at', function (Orders $shipment) {
    ...
    public function orders_datatable($type = null, $userid, Request $request)
    {
        if ($request->ajax()) {
            //settings
            $role = Auth()->user()->role;
            ...
            return DataTables::of($shipments)
                ...
                ->editColumn('code', function (Orders $shipment) {
                    $actionurl = "Shipment for - {$shipment->code}";
                    return $actionurl;
                })

                ->editColumn('status', function (Orders $shipment) {
                    $status_name = get_status('orders', $shipment->status);
                    $status = "<div class='status'><span class='badge bg-" . get_status_color($shipment->status, 'orders') . "-subtle text-" . get_status_color($shipment->status, 'orders') . " font-12'>{$status_name}</span></div>";
                ...
                ->editColumn('payment_method', function (Orders $shipment) {
                    $name =__('messages.' . get_data_db('payment_methods', 'id', $shipment->payment_method, 'name'));
                    return $name;
                })
                ->editColumn('owner_id', function (Orders $shipment) {
                    $user = User::where('id', $shipment->owner_id)->first();
                ...
                ->rawColumns(['invoice_id', 'code', 'status', 'shipping_cost', 'owner_id', 'payment_status', 'payment_method', 'created_at', 'action'])
                ->make(true);
        }
    }
