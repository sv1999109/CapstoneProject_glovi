    ...
    <div class="row">
        <div class="col-md-12x">
            <div class="card">
                ...
                <form id="create_form" data-action="{{ route('dashboard.location.update', ['type' => $type, 'id' => $id]) }}" class="form"
                    method="post">
                    @csrf
                    @method('POST')
                    <div class="card-body">
                        <div class="col-md-12">
                            ...
                            @if ($type == 'country')
                                <div class="col-md-6" style="float: none; margin: auto;">
                                    ...
                                    <div class="form-group mb-3">
                                        <label for="iso2" class="form-label text-uppercase required">@lang('iso2')</label>
                                        <input type="text" name="iso2" value="{{ old('iso2', $model->iso2) }}"
                                            maxlength="2" class="form-control @error('iso2') is-invalid @enderror"
                                            required>
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="iso3" class="form-label text-uppercase optional">@lang('iso3')
                                        </label>
                                        <input type="text" name="iso3" value="{{ old('iso3', $model->iso3) }}"
                                            maxlength="3" class="form-control @error('iso3') is-invalid @enderror">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="phone_code" class="form-label text-uppercase optional">@lang('messages.Phone_Code')
                                        </label>
                                    ...
                                    <div class="form-group mb-3">
                                        <label for="region" class="form-label">@lang('messages.Region')
                                        </label>
                                        <input type="text" name="region" value="{{ old('region', $model->region) }}"
                                            maxlength="3" class="form-control @error('region') is-invalid @enderror">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="subregion" class="form-label optional">@lang('messages.Subregion')
                                        </label>
                                        <input type="text" name="subregion"
                                            value="{{ old('subregion', $model->subregion) }}" maxlength="3"
                                            class="form-control @error('subregion') is-invalid @enderror">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="status" class="form-label optional">@lang('messages.Status')</label>
                                        <select name="status" class="form-select">
                                            <option value="1" {{ old('status') == 1 ? 'selected' : '' }}
                                                {{ $model->status == 1 ? 'selected' : '' }}>
                                                {{ get_status('', '1') }}</option>
                                            <option value="0" {{ old('status') == 2 ? 'selected' : '' }}
                                                {{ $model->status == 0 ? 'selected' : '' }}>
                                                {{ get_status('', '2') }}</option>
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group mb-3">
                                                ...
                                                <select id="sel_state" name="state"
                                                    ...
                                                    @foreach (DB::table('states')->whereRaw("country_id = '$model->country_id' AND status = 1")->get() as $item)
                                                        <option value="{{ __($item->id) }}"
                                                            {{ old('state') == $item->id ? 'selected' : '' }}
                                                            {{ $model->state_id == $item->id ? 'selected' : '' }}>
                                                            {{ $item->name }}
                                                        </option>
                                                    @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#create_form').submit(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('dashboard.location.update', ['type' => $type, 'id' => $id]) }}",
                    data: $form.serialize(),
                    success: save_data,
                    dataType: 'json',
                    error: function() {
