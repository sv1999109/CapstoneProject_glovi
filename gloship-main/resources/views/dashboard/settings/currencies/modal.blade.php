...
@if (isset($languages))
                    ...
                    <form action="{{ route('dashboard.languages.update', ['id' => $lang->id]) }}" class="form"
                        ...
                        <div class="modal-body">
                            <div class="col-md-12">
                                ...
                                <div class="form-group mb-3">
                                    <label for="native" class="form-label">Flag Code</label>
                                    <input type="text" name="flag_code" value="{{ $lang->flag_code }}"
                                        class="form-control @error('flag_code') is-invalid @enderror" required>
                                    <small id="helpId" class="form-text text-success">i.e: fr, us ,gb , cn, es...</small>
                                </div>

                                <div class="form-group mb-3">
                                    <label for="native" class="form-label">Status</label>
                                    <select name="status" class="form-select">
                                        <option value="1" @if ($lang->status == 1)
                                            selected
                                        @endif>{{ get_status('', '1') }}</option>
                                        <option value="2" @if ($lang->status == 2)
                                            selected
                                        @endif>{{ get_status('', '2') }}</option>
                                    </select>
                                </div>
    ...
    {{-- create --}}
    <div class="modal fade text-left" id="lang-create" tabindex="-1"
        aria-labelledby="myModalLabel110" aria-modal="true" role="dialog">
        <div class="modal-lg-x modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <form action="{{ route('dashboard.languages.create') }}" class="form"
                    method="post">
                    ...
                    <div class="modal-body">
                        <div class="col-md-12">
                            ...
                            <div class="form-group mb-3">
                                <label for="code" class="form-label">@lang('messages.Code')</label>
                                <input type="text" name="code" value=""
                                    class="form-control" required>
                                
                            </div>
                            <div class="form-group mb-3">
                                <label for="native" class="form-label">@lang('messages.Native_Name')</label>
                                <input type="text" name="native" value=""
                                    class="form-control" required>
                            </div>

                            <div class="form-group mb-3">
                                <label for="native" class="form-label">@lang('messages.Flag_Code') </label>
                                <input type="text" name="flag_code" value=""
                                    class="form-control" required>
                                <small id="helpId" class="form-text text-success">i.e: fr, us ,gb , cn, es...</small>
                            </div>
